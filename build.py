  ...
  <div class="container footer-grid">
    ...
    <nav class="footer-col" aria-label="서비스 안내">
      ...
      <ul>
        ...
        <li><a href="/gangnam/districts/">생활권별 안내</a></li>
...
def build() -> None:
    ...
    for p, c, r, d in sorted(report):
        flag = "" if (r == "noindex" or MIN_INDEX_CHARS <= c <= 2500) else "  ⚠본문"
        dflag = "" if d <= MAX_DESC_CHARS else f"  ⚠{d}자"
        if d > MAX_DESC_CHARS:
            over_desc.append((p, d))
        print(f"{p.ljust(width)}  {str(c).rjust(5)}  {r.ljust(8)}  {str(d).rjust(2)}자{dflag}{flag}")
    print(f"\n{len(report)} pages built, {len(sitemap_urls)} in sitemap.")
    if over_desc:
        print(f"\n⚠ 디스크립션 {MAX_DESC_CHARS}자 초과 {len(over_desc)}건:")
        for p, d in over_desc:
            print(f"  - {p} ({d}자)")
    else:
